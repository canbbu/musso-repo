...
import { useMySoccerAccessRequest } from '../hooks/use-soccer-access-requests';
import { useSportAccess } from '../hooks/use-sport-access';

/** 축구 권한 없을 때: 안내 + 권한 요청 버튼 */
export function SoccerAccessDenied() {
  const { hasPending, loading, submitting, submitRequest } = useMySoccerAccessRequest();
  ...
  return (
    <Layout>
      <div className="flex flex-col items-center justify-center min-h-[50vh] gap-6 p-6 max-w-md mx-auto">
        <p className="text-lg font-medium text-center">축구 페이지 접근 권한이 없습니다.</p>
        <p className="text-muted-foreground text-sm text-center">
          관리자(system-manager)가 승인하면 이용할 수 있습니다.
        </p>
        {loading ? (
